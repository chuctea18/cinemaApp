...
function SignUp() {
  ...
  const onSubmit = async () => {
    ...
    const body_data = { email, name, password, birth, phone };
  ...
  return (
    <div class="register_container" id="container" style={{ background: "#0000" }}>
      <div class="form-container log-in-container" style={{ marginTop: '3%' }}>
        <form className="form" style={{ borderRadius: '30px' }}>
          ...
          <span style={{ marginBottom: '10px' }}>Hoặc sử dụng tài khoản của bạn</span>
          <input style={{ marginBottom: '10px' }} type="text" placeholder="Tên đăng nhập" id="name" className="input" />
          <input style={{ marginBottom: '10px' }} type="email" placeholder="Email" id="email" className="input" />
          <input style={{ marginBottom: '10px' }} type={showPassword ? "text" : "password"} placeholder="Mật khẩu" id="password" className="input" />
          <img
            src={visibilityIcon}
            alt="show password"
          ...
          <input
            ...
            className="input"
          />
          <a href="/sign-in" style={{ fontSize: '14px', marginTop: '15px', marginBottom: '10px', color: 'black' }}><i>Bạn đã có tài khoản?</i></a>
          <button style={{ marginBottom: '20px', minHeight: '50px', minWidth: '100px', background: '#f18720', borderRadius: "10px", fontSize: '16px' }} onClick={onSubmit}>Đăng ký</button>
        </form>
      </div>
